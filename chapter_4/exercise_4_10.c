...
int getop(char sToken[])
{
    ...
    int iPointer=0; // pointing to the position of sTemp
    if (sTemp[iPointer] == EOF){
    	return EOF;
    }
    

    // Skip whitespace characters
    while ((sToken[0] = iCharacter = sTemp[++iPointer]) == ' ' || iCharacter == '\t')
	    ;
    ...
    {
        while (isdigit(sToken[++iIndex] = iCharacter = sToken[++iPointer]))
            ;
    }
...
/*
 * getline: read the line into the string.
 * Author: Manthan Nagar
 * Created: 16 July 2024
 * Modified: 16 July 2024
*/

void getline(char sToken[],int iLimit){
	char cCharacter; // temporary store of the current character
	int iIndex; // poitner to the index of the sToken string
	while (((cCharacter = getchar()) != EOF) && && (iIndex<iLimit)){
